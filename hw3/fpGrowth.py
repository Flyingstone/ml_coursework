...
#modified by LPZ 20171227
#to solve this error:
#RuntimeError: dictionary changed size during iteration
#    for k in headerTable.keys():  #remove items not meeting minSup
#        if headerTable[k] < minSup: 
#            del(headerTable[k])
    headerTable = {key:value for key,value in headerTable.items() if value >= minSup}
...
#不把重复值加入freqItemList
        ...
        if myHead != None: #3. mine cond. FP-tree
            #print ('conditional tree for: set',newFreqSet)
            #myCondTree.disp(1)            
            mineTree(myCondTree, myHead, minSup, newFreqSet, freqItemList)

def teamSelect(minNum,freqItems):
    temp = []
    for team in freqItems:
        if(len(team) >= minNum):
            temp.append(team)
    return temp
